...
function puctWinRate(config: RunConfig, iters: number, hybrid: boolean): number {
  let wins = 0;
  for (const seed of seeds) {
    const rng = new Rng(seedFromString(`hyb-${iters}-${hybrid}`));
...
for (const d of DIFFICULTIES) {
  ...
  for (const iters of ITERS) {
    console.log(`  net-PUCT  ${iters}:    ${(puctWinRate(config, iters, false) * 100).toFixed(1)}%`);
    console.log(`  hybrid    ${iters}:    ${(puctWinRate(config, iters, true) * 100).toFixed(1)}%`);
  }
}
